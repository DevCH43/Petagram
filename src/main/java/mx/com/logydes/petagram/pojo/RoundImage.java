...
public class RoundImage extends ImageView {
    ...
    public RoundImage(final Context context) {
        this(context, null);
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.RoundImage, defStyleAttr, 0);

        // Init Border
        if (attributes.getBoolean(R.styleable.RoundImage_civ_border, true)) {
            float defaultBorderSize = DEFAULT_BORDER_WIDTH * getContext().getResources().getDisplayMetrics().density;
            setBorderWidth(attributes.getDimension(R.styleable.RoundImage_civ_border_width, defaultBorderSize));
            setBorderColor(attributes.getColor(R.styleable.RoundImage_civ_border_color, Color.WHITE));
        }

        // Init Shadow
        if (attributes.getBoolean(R.styleable.RoundImage_civ_shadow, false)) {
            shadowRadius = DEFAULT_SHADOW_RADIUS;
            drawShadow(attributes.getFloat(R.styleable.RoundImage_civ_shadow_radius, shadowRadius), attributes.getColor(R.styleable.RoundImage_civ_shadow_color, shadowColor));
        }
    }
    //endregion
